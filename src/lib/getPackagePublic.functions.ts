  ...
  .handler(async ({ data }) => {
      ...
      .select(
        `id, token, organization_id, candidate_name, candidate_email, expires_at, opened_at,
         ...
         packages (
           ...
           equity_is_listed, equity_ticker, equity_company_valuation,
           equity_total_shares, equity_last_round_date,
           equity_scenario_bear, equity_scenario_base, equity_scenario_bull,
           equity_last_price, equity_last_price_currency, equity_price_fetched_at,
           ...
           equity_devices (
             ...
             special_conditions, vesting_schedule,
    ...
    return {
      ...
      package: {
        ...
        equity_is_listed: !!(pkg as any).equity_is_listed,
        equity_ticker: (pkg as any).equity_ticker ?? null,
        equity_company_valuation: (pkg as any).equity_company_valuation ?? null,
        equity_total_shares: (pkg as any).equity_total_shares ?? null,
        equity_last_round_date: (pkg as any).equity_last_round_date ?? null,
        equity_scenario_bear: Number((pkg as any).equity_scenario_bear) || 1.0,
        equity_scenario_base: Number((pkg as any).equity_scenario_base) || 3.0,
        equity_scenario_bull: Number((pkg as any).equity_scenario_bull) || 7.0,
        equity_last_price: (pkg as any).equity_last_price !== null
          ? Number((pkg as any).equity_last_price)
          : null,
        equity_last_price_currency: ((pkg as any).equity_last_price_currency as "EUR" | "USD" | null) ?? "EUR",
        equity_price_fetched_at: (pkg as any).equity_price_fetched_at ?? null,
